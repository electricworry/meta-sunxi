...
LIC_FILES_CHKSUM = "file://${S}/LICENSE;md5=e8c1458438ead3c34974bc0be3a03ed6"
...
do_install () {
	install -d ${D}${sbindir}
    install -m 700 \
		${S}/external/config/optional/families/sun50iw9/_packages/bsp-cli/usr/bin/${INITSCRIPT_NAME} \
		${D}${sbindir}/${INITSCRIPT_NAME}
    install -m 700 ${UNPACKDIR}/${INITSCRIPT_NAME}.sh ${D}${sbindir}/${INITSCRIPT_NAME}.sh

	if ${@bb.utils.contains('DISTRO_FEATURES', 'sysvinit', 'true', 'false', d)}; then
		install -d ${D}${sysconfdir}/init.d/
		install -m 755 ${UNPACKDIR}/${INITSCRIPT_NAME}.init ${D}${sysconfdir}/init.d/${INITSCRIPT_NAME}
	...
	if ${@bb.utils.contains('DISTRO_FEATURES', 'systemd', 'true', 'false', d)}; then
		...
		install -m 644 ${UNPACKDIR}/${INITSCRIPT_NAME}.service ${D}${systemd_system_unitdir}/${INITSCRIPT_NAME}.service
		sed -i -e 's,@SBINDIR@,${sbindir},g' \
